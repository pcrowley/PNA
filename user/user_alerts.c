...
void pna_alert_recv(struct pna_alert_msg *alert)
{
    ...
    proto_i =
	(alert->reason & PNA_ALERT_PROTO_MASK) >> PNA_ALERT_PROTO_SHIFT;
...
//    snprintf(tstamp, MAX_STR, "%u", alert->timeval.tv_sec);
    ...
    if (out_file != NULL) {
	fprintf(out_file, PNA_ALERT_LOG_FORMAT, tstamp, type, proto, dir,
		ip);
    }
	...
	if (fork() == 0) {
	    /* child can exec */
	    if (execl
		(handler_path, handler_path, type, proto, dir, ip,
		 (char *) NULL) < 0) {
...
void usage(void)
{
    ...
    printf
	("You must specify <logfile> and/or <handler> to run this program\n");
...
int main(int argc, char **argv)
{
    ...
    if (bind(socket_fd, (struct sockaddr *) &pna_user, sizeof(pna_user)) <
	0) {
	perror("bind");
	return -1;
    }
