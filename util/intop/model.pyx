...
# raw_data is formatted as follows (generated by PNALogParser)
# [ { end-time:int,
#     [ watch-data:{ local-ip:int,
#                    remote-ip:int,
#                    ntcp:int,
#                    nudp:int,
#                    tcp-tuples:[ { local-port:short,
#                                   remote-port:short,
#                                   npkts-in:int,
#                                   npkts-out:int,
#                                   nbytes-in:int,
...
class PNAModel :
    ...
    filters    = ('local-ip','remote-ip','begin-time','end-time',  'latest',)
    filter_res = (     ip_re,      ip_re,     time_re,   time_re, latest_re,)
    ...
    def filter_reject(self, f_name, value) :
        ...
        if f_name == 'local-ip' or f_name == 'remote-ip' :
            ...
            if f_name == 'begin-time' :
                ...
                month = int(month[1:]) if month != None else dt_date.min.month
                day = int(day[1:]) if day != None else dt_date.min.day
                hour = int(hour[1:]) if hour != None else dt_time.min.hour
                min = int(min[1:]) if min != None else dt_time.min.minute
                sec = int(sec[1:]) if sec != None else dt_time.min.second
                ...
                month = int(month[1:]) if month != None else dt_date.max.month
                day = int(day[1:]) if day != None else dt_date.max.day
                hour = int(hour[1:]) if hour != None else dt_time.max.hour
                min = int(min[1:]) if min != None else dt_time.min.minute
                sec = int(sec[1:]) if sec != None else dt_time.max.second
                # datetime-ify the filter value
                f_dt = datetime(year,month,day,hour,min,sec)
                # check the values
                if f_dt < v_dt :
    ...
    def do_filter(self, raw_data) :
        new_data = [ ]

        # we'll go through all the raw_data and filter out items we don't want
        for item in raw_data :
            ...
            for entry in watch_data :
                ...
                if self.filter_reject('local-ip', local_ip) :
                    # skip this entire entry (don't add to new_watch_data)
                    continue
                remote_ip = entry['remote-ip']
                ...
                for tuple in tcp_tuples :
                    begin_time = tuple['begin-time']
                    if self.filter_reject('begin-time', begin_time) :
                        continue
                    # add to new_tcp_tuples
                    new_tcp_tuples.append(tuple)
                for tuple in udp_tuples :
                    begin_time = tuple['begin-time']
                    if self.filter_reject('begin-time', begin_time) :
                        continue
                    # add to new_udp_tuples
                    new_udp_tuples.append(tuple)
                # now combine all filtered data for an entry
                if len(new_tcp_tuples) + len(new_udp_tuples) == 0 :
                    continue
                new_entry = { }
                new_entry['local-ip'] = local_ip
                new_entry['remote-ip'] = remote_ip
                new_entry['ntcp'] = len(new_tcp_tuples)
                new_entry['nudp'] = len(new_udp_tuples)
                new_entry['tcp-tuples'] = new_tcp_tuples
                new_entry['udp-tuples'] = new_udp_tuples
                new_watch_data.append(new_entry)
            # now combine all the filtered data for a item
            new_item = { }
            new_item['end-time'] = end_time
            new_item['watch-data'] = new_watch_data
            new_data.append(new_item)
        return new_data


    # should parse a file and add it to the raw_data structure
    def add_file(self, file_name) :
        file_data = PNALogParser.parse(file_name)
        self.raw_data.extend(file_data)
        #self.raw_data = file_data

    # derives a raw_data list to a local->remote list
    # returns
    # { 'local-ip', 'remote-ip', 'all-ports', 'all-packets', 'all-octets',
    #   'tcp-ports', 'tcp-packets','tcp-octets', 'udp-ports', 'udp-packets',
    #   'udp-octets' }
    def derive_point_to_points(self, raw_data) :
        # this is gonna be intense
        data = { } # we want to combine like point-to-point connections

        # we need to derive from every item in the raw_data list
        for item in raw_data :
            end_time = item['end-time']
            watch_data = item['watch-data']
            # each item has multiple data entries for a dump-/end-time
    ...
    # derives a raw_data list to a local-ip only list
    # returns
    # { 'local-ip', 'connections', 'sessions', 'all-packets', 'all-octets',
    #   'tcp-ports', 'tcp-packets','tcp-octets', 'udp-ports', 'udp-packets',
    #   'udp-octets' }
    def derive_local_ips(self, raw_data) :
        #raw_data = self.derive_point_to_points(raw_data)
    ...
    def get_data(self, raw=False) :
        ...
        # filter the raw_data through any filters
        data = self.raw_data
        data = self.do_filter(self.raw_data)
        ...
        if raw == True :
            ...
            data = sorted(data, lambda l,r : cmp(l['end-time'], r['end-time']))
            data.reverse()
            flat_data = [ ]
        ...
        data = sorted(data, lambda l,r : cmp(l[key], r[key]))
        # move from descending to ascending
        data.reverse()
    ...
    def get_fields(self, headers=True) :
        ...
        if key == 'raw' :
            ...
            if headers == True :
                fields.extend(list(eval(key[0:3]+'_fields', locals())))
            else :
                fields.extend(list(tcp_fields))
                fields.extend(list(udp_fields))
